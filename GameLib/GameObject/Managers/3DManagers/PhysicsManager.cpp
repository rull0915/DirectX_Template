...
void PhysicsManager::HittedCorrection()
{
	...
	for (auto& contact : m_contacts)
	{
		...
		// リストに追加
		ObjectPair pair = ObjectPair(contact.a, contact.b);
...
{
	auto aRigid = contact.aRigid, bRigid = contact.bRigid;

	// 動く割合の算出
	float ratioA = 0, ratioB = 0;
	{
		// どちらかがStaticな場合
		if (contact.aIsStatic) {
			ratioA = 0;   // Aは動かない
			ratioB = 1;   // Bが100%押し戻される
		}
		else if (contact.bIsStatic) {
	...
	// ----- 位置の補正 ----- //
	{
		...
		contact.a->GetComponent<Transform>()->AddCache(corrVecA);
		contact.b->GetComponent<Transform>()->AddCache(corrVecB);
	}
}
...
void PhysicsManager::VelocityCorrection(HitContact& contact)
{
	...
	if (hitDirVel < 0)
	{
		...
		// 双方の速度を変更
		if (aRigid && !contact.aIsStatic) aRigid->AddForce(-(tangentImpulse + normalImpulse), ForceMode::Impulse, false);
		if (bRigid && !contact.bIsStatic) bRigid->AddForce(tangentImpulse + normalImpulse, ForceMode::Impulse, false);
	}
}
